...
for spec in \
  "preamble:$((preamble_bytes / 2)):16" \
  "sync:$preamble_bytes:4" \
  "header:$data_start:16" \
  "data:$((data_start + 4096)):16" \
  "data_large:$((data_start + 8192)):64"; do
  IFS=: read -r label offset burst <<< "$spec"
  test_file="$TEST_TMPDIR/${label}.pcm"
  cp "$encoded" "$test_file"
  corrupt_burst "$test_file" "$offset" "$burst"
  if roundtrip_ok "$payload" "$test_file" "$decoded"; then
    echo "OK $label burst=${burst}B"
  else
    echo "FAIL $label burst=${burst}B"
    exit 1
  fi
done

echo "Chirp corruption test OK"
